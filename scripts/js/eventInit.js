...
function showCharRange(case1,range,rangeattack){
    var checkPath;
    for(var i=-range;i<=range;i++)
    {
        ...
        rangeattack = 3
        for(var j=-(rangeattack);j<=rangeattack;j++)
        {
            if(Math.abs(i)+Math.abs(j)<=rangeattack)
            {
                if(j+case1.y>=0 && j+case1.y<mapParams.nbCaseMapY)
                {
                   // console.log(i+case1.x,j+case1.y)
                    ...
                    checkPath = finder.findPath(case1.x, case1.y, i+case1.x,j+case1.y , grid);
                    grid = gridBackup;
                    if(checkPath.length<=rangeattack+1 && map1["collisions"][j+case1.y][i+case1.x]!=1)
                    {
